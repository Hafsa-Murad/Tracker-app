  ...
  return (
    <div className="flex flex-col items-center mb-8 ">
      ...
      <input
        ...
        className="border p-2 rounded w-full sm:w-48 mb-4 text-lg lg:w-1/2 xl:w-1/3 " // Increase text size in input field
      />
      {searchResults.length > 0 && (
        <div className="border rounded w-full sm:w-48 bg-white lg:w-1/2 xl:w-1/3 ">
          {searchResults.map((result) => (
            <div key={result.id} className="p-4 border-b last:border-b-0 flex items-center justify-between"> {/* Increased padding */}
              ...
              <span>Quantity: {result.quantity}</span> {/* Increased font size */}
